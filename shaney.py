...
def main():
    ...
    for filename in sorted(os.listdir(data_dir)):
        ...
        # FIX 1: use 'with open' to properly close file
        with open(data_dir + filename, encoding="utf-8") as f:
            words = f.read().split()

        # FIX 2: guard against files with fewer than n words
        if len(words) >= n:
            # FIX 3: use 'n' instead of hardcoded 2
    ...
    # FIX 4: use 'with open' for output file as well
